...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if 'lm_head.weight' in model.state_dict():
            self.ref_head = model.state_dict()['lm_head.weight']
    ...
    def get_batch_metrics(
        ...
        output_accuracies = (policy_chosen_logps > policy_rejected_logps).float()

        prefix = "eval_" if train_eval == "eval" else ""

        metrics[f"{prefix}rewards/chosen"] = chosen_rewards.detach().mean()
        metrics[f"{prefix}rewards/rejected"] = rejected_rewards.detach().mean()
        metrics[f"{prefix}rewards/accuracies"] = reward_accuracies.detach().mean()
        metrics[f"{prefix}outputs/accuracies"] = output_accuracies.detach().mean()
        metrics[f"{prefix}rewards/margins"] = (chosen_rewards - rejected_rewards).detach().mean()
    ...
    def compute_loss(
        ...
        total_loss, metrics = self.accelerator.gather_for_metrics((loss, metrics))
    ...
    def prediction_step(
        ...
        loss, metrics = self.accelerator.gather_for_metrics((loss, metrics))
        loss = loss.mean()
        for k in metrics:
            metrics[k] = metrics[k].mean()
        # force log the metrics
